...
lme.cog3<-mixed_model(cognitive~study+agebl+sex+yearseducation+smoking,random = ~ followupyears | idpicc, data=data_long.fit.up,family = binomial() )
...
baseline.cox.up1<-coxph(Surv(years,cens)~agebl+sex+yearseducation+mdsupdrspart3bltotalconvertedasa+hybl+study,data=data.baseline.fit.up, x = TRUE,model = TRUE)

baseline.cox.up2<-coxph(Surv(years,cens)~agebl+sex+yearseducation+mdsupdrspart3bltotalconvertedasa+hybl+study+smoking,data=data.baseline.fit.up, x = TRUE,model = TRUE)
...
Mixed<-list(lme.up,lme.cog2) #lme.hall1

jointFit.up1<-jm(baseline.cox.up1,Mixed_objects = Mixed,time_var = "followupyears")

jointFit.up2<-jm(baseline.cox.up2,Mixed_objects = Mixed,time_var = "followupyears")

compare_jm(jointFit.up1,jointFit.up2)
...
predLong.2<-predict(jointFit.up1,newdata = predict.data.time.2,times = seq(3,6, length.out=20),return_newdata = T) #for future time point

predSurv.2<-predict(jointFit.up1,newdata = predict.data.time.2,process = "event",times = seq(3,6, length.out=20),return_newdata = T) # use the information before 3 years to predict
...
predLong.4<-predict(jointFit.up1,newdata = predict.data.time.4,times = seq(4.9,7.9, length.out=20),return_newdata = T) #for future time point

predSurv.4<-predict(jointFit.up1,newdata = predict.data.time.4,process = "event",times = seq(4.9,7.9, length.out=20),return_newdata = T) # use the information before 3 years to predict

png("JMpred.Cam4.png",width = 2000,height =3000,res = 400)
plot(predLong.4,predSurv.4,outcomes=1:2,
     ylab_event="Cumulative risk of dementia",
     ylab_long=c("cognitive symptom","MMSE"),
     xlab = "Follow-up years",cex_ylab_long = 1.5,cex_axis = 1.2,cex_xlab = 1)
...
predLong.5<-predict(jointFit.up1,newdata = predict.data.time.5,times = seq(6.8,9.8, length.out=20),return_newdata = T) #for future time point

predSurv.5<-predict(jointFit.up1,newdata = predict.data.time.5,process = "event",times = seq(6.8,9.8, length.out=20),return_newdata = T) # use the information before 3 years to predict

png("JMpred.Cam5.png",width = 2000,height =3000,res = 400)
plot(predLong.5,predSurv.5,outcomes=1:2,
     ylab_event="Cumulative risk of dementia",
     ylab_long=c("cognitive symptom","MMSE"),
     xlab = "Follow-up years",cex_ylab_long = 1.5,cex_axis = 1.2,cex_xlab = 1)
...
predLong.1<-predict(jointFit.up1,newdata = predict.data.time.1,times = seq(1,4, length.out=20),return_newdata = T) #for future time point

predSurv.1<-predict(jointFit.up1,newdata = predict.data.time.1,process = "event",times = seq(1,4, length.out=20),return_newdata = T) # use the information before 3 years to predict
...
predLong.5<-predict(jointFit.up1,newdata = predict.data.time.5,times = seq(5,8, length.out=20),return_newdata = T) #for future time point

predSurv.5<-predict(jointFit.up1,newdata = predict.data.time.5,process = "event",times = seq(5,8, length.out=20),return_newdata = T) # use the information before 3 years to predict
