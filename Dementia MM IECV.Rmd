...
#Error in lme.formula(mmse ~ study + agebl + sex + yearseducation + cognitivebl +  : 
...
#remove follow-up years still cannot convergent
...
mmse.PP<-lme(mmse~agebl+sex+yearseducation+cognitivebl+rcs(followupyears,3), random = ~ followupyears | idpicc, data=IECV_vlong_PP) #see take rcs out will be better or not
...
Cox_PP<-coxph(Surv(years,cens)~agebl+sex+hybl+cognitivebl,data=IECV_vbase_PP, x = TRUE,model = TRUE) #take out hallucination to see if it will be better #+hallucinationsbl
...
#If take out rcs(,3), the Rhat in MDS follow-up year will be 1.1 and the tAUC value in CampalGN is not good
...
Cox_Cam<-coxph(Surv(years,cens)~agebl+sex+hybl+cognitivebl,data=IECV_vbase_Cam, x = TRUE,model = TRUE)
#cox.zph(Cox_Cam) all pass #take out +hallucinationsbl
...
Cox_PIC<-coxph(Surv(years,cens)~agebl+sex+hybl+cognitivebl,data=IECV_vbase_PIC, x = TRUE,model = TRUE) #take out +hallucinationsbl
...
```{r model developement from CamPalGN extern validate in PICNICS, develop from PICNICS external validate CamPalGN}
...
```{r CamPalGN and PICNICS gap between revisit}

#----CamPalGN----

table(IECV_vlong_Cam$study,IECV_vlong_Cam$visityear)

#First revisit is at year2, 16 patients

`%!in%` = Negate(`%in%`)

Cam_reyear2idpicc<-IECV_vlong_Cam%>%
  filter(visityear=="year2")


Cam_reyear3idpicc<-IECV_vlong_Cam%>%
  filter(idpicc!=Cam_reyear2idpicc$idpicc)%>%
  filter(visityear=="year3")

#First revisit is at year3, 50 patients


Cam_reyear4idpicc<-IECV_vlong_Cam%>%
  filter(idpicc%!in% c(Cam_reyear2idpicc$idpicc,Cam_reyear3idpicc$idpicc))%>%
  filter(visityear=="year4")

#First revisit is at year4, 35 patients

Cam_reyear5idpicc<-IECV_vlong_Cam%>%
  filter(idpicc%!in% c(Cam_reyear2idpicc$idpicc,Cam_reyear3idpicc$idpicc,Cam_reyear4idpicc$idpicc))%>%
  filter(visityear=="year5")

#First revisit is at year 5, 4 patients

IECV_vlong_Cam%>%
  filter(idpicc%!in% c(Cam_reyear2idpicc$idpicc,Cam_reyear3idpicc$idpicc,Cam_reyear4idpicc$idpicc,Cam_reyear5idpicc$idpicc))%>%
  filter(visityear=="year6")

#zero patients

#see how many people develop dementia before year 3

IECV_vbase_Cam%>%
  filter(cens==1)%>%
  filter(years<5)

#41 event, 25 before year 5

#----PICNICS----

table(IECV_vlong_PIC$study,IECV_vlong_PIC$visityear)

#First revisit is at year1, 25 patients

`%!in%` = Negate(`%in%`)

PIC_reyear1idpicc<-IECV_vlong_PIC%>%
  filter(visityear=="year1")


PIC_reyear2idpicc<-IECV_vlong_PIC%>%
  filter(idpicc %!in% c(PIC_reyear1idpicc$idpicc))%>%
  filter(visityear=="year2")

#First revisit is at year2, 131 patients


PIC_reyear3idpicc<-IECV_vlong_PIC%>%
  filter(idpicc%!in% c(PIC_reyear1idpicc$idpicc,PIC_reyear2idpicc$idpicc))%>%
  filter(visityear=="year3")

#First revisit is at year3, 20 patients

PIC_reyear4idpicc<-IECV_vlong_PIC%>%
  filter(idpicc%!in% c(PIC_reyear1idpicc$idpicc,PIC_reyear2idpicc$idpicc,PIC_reyear3idpicc))%>%
  filter(visityear=="year4")

#First revisit is at year4, 11 patients

PIC_reyear5idpicc<-IECV_vlong_PIC%>%
  filter(idpicc%!in% c(PIC_reyear1idpicc$idpicc,PIC_reyear2idpicc$idpicc,PIC_reyear3idpicc,PIC_reyear4idpicc$idpicc))%>%
  filter(visityear=="year5")

#First revisit is at year5, 7 patients

IECV_vlong_PIC%>%
  filter(idpicc%!in% c(PIC_reyear1idpicc$idpicc,PIC_reyear2idpicc$idpicc,PIC_reyear3idpicc,PIC_reyear4idpicc$idpicc,PIC_reyear5idpicc$idpicc))%>%
  filter(visityear=="year5")
...
#51 event, 32 before year 3

32/51

```

```{r actual tAUC and calibration plot for ParkWest}

#-----predictive performance at original data set-----

roc.pa.or.0<-tvROC(jm_dev.PP,newdata=IECV_vlong_PA,Tstart = 0,Dt=3,cores=1L)
roc.pa.or.1<-tvROC(jm_dev.PP,newdata=IECV_vlong_PA,Tstart = 1,Dt=3,cores=1L)
roc.pa.or.2<-tvROC(jm_dev.PP,newdata=IECV_vlong_PA,Tstart = 2,Dt=3,cores=1L)
roc.pa.or.3<-tvROC(jm_dev.PP,newdata=IECV_vlong_PA,Tstart = 3,Dt=3,cores=1L)
roc.pa.or.4<-tvROC(jm_dev.PP,newdata=IECV_vlong_PA,Tstart = 4,Dt=3,cores=1L)

tvAUC(roc.pa.or.0) #0.6302
tvAUC(roc.pa.or.1) #0.6475
tvAUC(roc.pa.or.2) #0.7176
tvAUC(roc.pa.or.3) #0.6844
tvAUC(roc.pa.or.4) #0.6356

#----calibration plot---

png("ParkWest.actual.calibration.png",width = 3000,height =2000,res = 400)
par(mfrow=c(2,3))
calibration_plot(jm_dev.PP,newdata = IECV_vlong_PA,Tstart = 0,Dt=3,plot = TRUE)
calibration_plot(jm_dev.PP,newdata = IECV_vlong_PA,Tstart = 1,Dt=3,plot = TRUE)
calibration_plot(jm_dev.PP,newdata = IECV_vlong_PA,Tstart = 2,Dt=3,plot = TRUE)
calibration_plot(jm_dev.PP,newdata = IECV_vlong_PA,Tstart = 3,Dt=3,plot = TRUE)
calibration_plot(jm_dev.PP,newdata = IECV_vlong_PA,Tstart = 4,Dt=3,plot = TRUE)
dev.off()
```

```{r Bootstrap ParkWest}

IECV_vbase_PA<-data.baseline.fit.up%>%
  filter(study %in% c("ParkWest"))

IECV_vbase_PA$study<-droplevels(IECV_vbase_PA$study)

#need to add one column id in the baseline and the longitudinal data

IECV_vbase_PA$id<-seq(nrow(IECV_vbase_PA)) 

IECV_vlong_PA<-merge(IECV_vlong_PA,IECV_vbase_PA[,c("idpicc","id")],by="idpicc",all.x = T)


table(IECV_vbase_PA$hallucinationsbl) #Only 2 patients have yes in hallucination, idpicc 513/566, id=72, 124

#As only 2 patients had hallucination at baseline and the bootstrap training dataset may all have zero hallucination at baseline. Therefore, for the bootstrap trainning, the Cox regression will without hallucination. Also hallucination was not significant in the model


#----Bootstrap dataset----
n<-dim(IECV_vbase_PA)[1]
tauc_opt.0<-numeric()
tauc_opt.1<-numeric()
tauc_opt.2<-numeric()
tauc_opt.3<-numeric()
...
for (i in 1:100) {
  ...
  for (j in 1:length(n_new)){
    n_len <- length(IECV_vlong_PA$id[IECV_vlong_PA$id %in% n_new[j]])
    newdata <- IECV_vlong_PA[IECV_vlong_PA$id %in% n_new[j],]
  ...
  fit1.mmse <- lme(mmse~agebl+sex+yearseducation+cognitivebl+rcs(followupyears,3), random = ~ followupyears | id,
                        data = trainingData)
  
  fit2.mds <- lme(MDSUPDRS3~agebl+sex+hybl+rcs(followupyears,3), random = ~ followupyears | id, data=trainingData)
  
  Mixed_fit12.PA<-list(fit1.mmse,fit2.mds)
  
  survFit1 <- coxph(Surv(years,cens)~agebl+sex+hybl+cognitivebl, 
                       data = trainingData.id, x = TRUE,model = TRUE)
  
  jointFit1 <- jm(survFit1,Mixed_objects=Mixed_fit12.PA, time_var = "followupyears") #I check there is no NA
  
  jm(survFit1,fit1.mmse, time_var = "followupyears")
  
  #The predictive measures on the bootstrap data from the model fitted on the bootstrap sample
  
  auc_boot.0 <- tvAUC(jointFit1,newdata=trainingData,Tstart = 0,Dt=3,cores=1L)$auc
  auc_boot.1 <- tvAUC(jointFit1,newdata=trainingData,Tstart = 1,Dt=3,cores=1L)$auc
  auc_boot.2 <- tvAUC(jointFit1,newdata=trainingData,Tstart = 2,Dt=3,cores=1L)$auc
  auc_boot.3 <- tvAUC(jointFit1,newdata=trainingData,Tstart = 3,Dt=3,cores=1L)$auc
  auc_boot.4 <- tvAUC(jointFit1,newdata=trainingData,Tstart = 4,Dt=3,cores=1L)$auc
  
  #the predictive measures on the original data from the model fitted on the bootstrap.
  
  auc_orig.0 <- tvAUC(jointFit1,newdata=IECV_vlong_PA,Tstart = 0,Dt=3,cores=1L)$auc
  auc_orig.1 <- tvAUC(jointFit1,newdata=IECV_vlong_PA,Tstart = 1,Dt=3,cores=1L)$auc
  auc_orig.2 <- tvAUC(jointFit1,newdata=IECV_vlong_PA,Tstart = 2,Dt=3,cores=1L)$auc
  auc_orig.3 <- tvAUC(jointFit1,newdata=IECV_vlong_PA,Tstart = 3,Dt=3,cores=1L)$auc
  auc_orig.4 <- tvAUC(jointFit1,newdata=IECV_vlong_PA,Tstart = 4,Dt=3,cores=1L)$auc
  
  # The optimism in the prediction performance is estimated by subtracting the measures boot and org.
  tauc_opt.0[i] <- auc_boot.0 - auc_orig.0
  tauc_opt.1[i] <- auc_boot.1 - auc_orig.1
  tauc_opt.2[i] <- auc_boot.2 - auc_orig.2
  tauc_opt.3[i] <- auc_boot.3 - auc_orig.3
  tauc_opt.4[i] <- auc_boot.4 - auc_orig.4
}
...
for (i in 1:100) {
  ...
  tauc_corr.0[i] <- tvAUC(roc.pa.or.0)$auc - tauc_opt.0[i]
  tauc_corr.1[i] <- tvAUC(roc.pa.or.1)$auc - tauc_opt.1[i]
  tauc_corr.2[i] <- tvAUC(roc.pa.or.2)$auc - tauc_opt.2[i]
  tauc_corr.3[i] <- tvAUC(roc.pa.or.3)$auc - tauc_opt.3[i]
  tauc_corr.4[i] <- tvAUC(roc.pa.or.4)$auc - tauc_opt.4[i]
}

mean(tauc_corr.0,na.rm = T)
mean(tauc_corr.1,na.rm = T)
mean(tauc_corr.2,na.rm = T)
mean(tauc_corr.3,na.rm = T)
mean(tauc_corr.4,na.rm = T)
...
```{r see how many event in each year}
...
#random select idpicc==378 is kind of ok


predict.data<-IECV_vlong_NY[IECV_vlong_NY$idpicc == 378,] #event at 3.5 years
...
predLong.0<-predict(jm_dev.NP,newdata = predict.data.time.0,times = seq(0,3, length.out=20),return_newdata = T) #for future time point
...
predict.data.time.1$years<-1.03 

predLong.1<-predict(jm_dev.NP,newdata = predict.data.time.1,times = seq(1.03,4.03, length.out=20),return_newdata = T) #for future time point

predSurv.1<-predict(jm_dev.NP,newdata = predict.data.time.1,process = "event",times = seq(1.03,4.03, length.out=20),return_newdata = T) # use the information before 3 years to predict
...
predict.data.time.2<-predict.data[predict.data$followupyears<2.1,] 

predict.data.time.2$cens<-0 #not event

predict.data.time.2$years<-2

predLong.2<-predict(jm_dev.NP,newdata = predict.data.time.2,times = seq(2,5.07, length.out=20),return_newdata = T) #for future time point

predSurv.2<-predict(jm_dev.NP,newdata = predict.data.time.2,process = "event",times = seq(2,5.07, length.out=20),return_newdata = T) # use the information before 3 years to predict

plot(predLong.2,predSurv.2,outcomes=1:2)

#------From baseline up to 3 years-----

predict.data.time.3<-predict.data[predict.data$followupyears<3.1,] 
...
predLong.3<-predict(jm_dev.NP,newdata = predict.data.time.3,times = seq(3.1,6.1, length.out=20),return_newdata = T) #for future time point

predSurv.3<-predict(jm_dev.NP,newdata = predict.data.time.3,process = "event",times = seq(3,6, length.out=20),return_newdata = T) # use the information before 3 years to predict

plot(predLong.3,predSurv.3,outcomes=1:2)

#----Arrange plot----

png("JMpred0.NYPUM.png",width = 3500,height =2000,res = 400)

plot(predLong.0,predSurv.0,outcomes=1:2,ylab_long = c(" "," "),ylab_event = "Cumulative risk of dementia",xlab = "Follow-up years",cex_ylab_long = 1.5,cex_axis = 1.2,cex_xlab = 1)

dev.off()

png("JMpred1.NYPUM.png",width = 3500,height =2000,res = 400)
plot(predLong.1,predSurv.1,outcomes=1:2,ylab_long = c(" "," "),ylab_event = "Cumulative risk of dementia",xlab = "Follow-up years",cex_ylab_long = 1.5,cex_axis = 1.2,cex_xlab = 1)
dev.off()

png("JMpred2.NYPUM.png",width = 3500,height =2000,res = 400)
plot(predLong.2,predSurv.2,outcomes=1:2,ylab_long = c(" "," "),ylab_event = "Cumulative risk of dementia",xlab = "Follow-up years",cex_ylab_long = 1.5,cex_axis = 1.2,cex_xlab = 1)
dev.off()

png("JMpred3.NYPUM.png",width = 3500,height =2000,res = 400)
plot(predLong.3,predSurv.3,outcomes=1:2,ylab_long = c(" "," "),ylab_event = "Cumulative risk of dementia",xlab = "Follow-up years",cex_ylab_long = 1.5,cex_axis = 1.2,cex_xlab = 1)
dev.off()

```


```{r predict individual in PINE}
...
#random select idpicc==1168 is kind of ok

predict.data<-IECV_vlong_PIN[IECV_vlong_PIN$idpicc == 1168,] #event at 9 years

levels(predict.data$study)<-c("PINE","NYPUM") #it will only take PINE level
...
predict.data.time.1$years<-1.02 

predLong.1<-predict(jm_dev.NP,newdata = predict.data.time.1,times = seq(1.02,4.03, length.out=20),return_newdata = T) #for future time point

predSurv.1<-predict(jm_dev.NP,newdata = predict.data.time.1,process = "event",times = seq(1.02,4.03, length.out=20),return_newdata = T) # use the information before 3 years to predict
...
predLong.2<-predict(jm_dev.NP,newdata = predict.data.time.2,times = seq(2,5, length.out=20),return_newdata = T) #for future time point
...
predict.data.time.3$years<-3 

#predict for longitudinal outcomes

predLong.3<-predict(jm_dev.NP,newdata = predict.data.time.3,times = seq(3,6, length.out=20),return_newdata = T) #for future time point
...
predict.data.time.4<-predict.data[predict.data$followupyears<4.1,] 

predict.data.time.4$cens<-0 #not event

predict.data.time.4$years<-4.02 

#predict for longitudinal outcomes

predLong.4<-predict(jm_dev.NP,newdata = predict.data.time.4,times = seq(4.02,7.02, length.out=20),return_newdata = T) #for future time point

predSurv.4<-predict(jm_dev.NP,newdata = predict.data.time.4,process = "event",times = seq(4.02,7.02, length.out=20),return_newdata = T) # use the information before 3 years to predict
...
predict.data.time.5<-predict.data[predict.data$followupyears<5.1,] 
...
predLong.5<-predict(jm_dev.NP,newdata = predict.data.time.5,times = seq(5.01,8, length.out=20),return_newdata = T) #for future time point

predSurv.5<-predict(jm_dev.NP,newdata = predict.data.time.5,process = "event",times = seq(5.01,8, length.out=20),return_newdata = T) # use the information before 3 years to predict

plot(predLong.5,predSurv.5,outcomes=1:2)

#------From baseline up to 6 years-----

predict.data.time.6<-predict.data[predict.data$followupyears<6,] 

predict.data.time.6$cens<-0 #not event

predict.data.time.6$years<-6

#predict for longitudinal outcomes

predLong.6<-predict(jm_dev.NP,newdata = predict.data.time.6,times = seq(6,9, length.out=20),return_newdata = T) #for future time point

predSurv.6<-predict(jm_dev.NP,newdata = predict.data.time.6,process = "event",times = seq(6,9, length.out=20),return_newdata = T) # use the information before 3 years to predict

plot(predLong.6,predSurv.6,outcomes=1:2)

#----Arrange plot----

png("JMpred0.PINE.png",width = 3500,height =2000,res = 400)

plot(predLong.0,predSurv.0,outcomes=1:2,ylab_long = c(" "," "),ylab_event = "Cumulative risk of dementia",xlab = "Follow-up years",cex_ylab_long = 1.5,cex_axis = 1.2,cex_xlab = 1)

dev.off()

png("JMpred1.PINE.png",width = 3500,height =2000,res = 400)

plot(predLong.1,predSurv.1,outcomes=1:2,ylab_long = c(" "," "),ylab_event = "Cumulative risk of dementia",xlab = "Follow-up years",cex_ylab_long = 1.5,cex_axis = 1.2,cex_xlab = 1)

dev.off()

png("JMpred2.PINE.png",width = 3500,height =2000,res = 400)

plot(predLong.2,predSurv.2,outcomes=1:2,ylab_long = c(" "," "),ylab_event = "Cumulative risk of dementia",xlab = "Follow-up years",cex_ylab_long = 1.5,cex_axis = 1.2,cex_xlab = 1)

dev.off()

png("JMpred3.PINE.png",width = 3500,height =2000,res = 400)

plot(predLong.3,predSurv.3,outcomes=1:2,ylab_long = c(" "," "),ylab_event = "Cumulative risk of dementia",xlab = "Follow-up years",cex_ylab_long = 1.5,cex_axis = 1.2,cex_xlab = 1)

dev.off()

png("JMpred4.PINE.png",width = 3500,height =2000,res = 400)

plot(predLong.4,predSurv.4,outcomes=1:2,ylab_long = c(" "," "),ylab_event = "Cumulative risk of dementia",xlab = "Follow-up years",cex_ylab_long = 1.5,cex_axis = 1.2,cex_xlab = 1)

dev.off()

png("JMpred5.PINE.png",width = 3500,height =2000,res = 400)

plot(predLong.5,predSurv.5,outcomes=1:2,ylab_long = c(" "," "),ylab_event = "Cumulative risk of dementia",xlab = "Follow-up years",cex_ylab_long = 1.5,cex_axis = 1.2,cex_xlab = 1)

dev.off()

png("JMpred6.PINE.png",width = 3500,height =2000,res = 400)

plot(predLong.6,predSurv.6,outcomes=1:2,ylab_long = c(" "," "),ylab_event = "Cumulative risk of dementia",xlab = "Follow-up years",cex_ylab_long = 1.5,cex_axis = 1.2,cex_xlab = 1)
...
```{r predict individual in CamPalGN}
...
#random select idpicc==38 is ok

predict.data<-IECV_vlong_Cam[IECV_vlong_Cam$idpicc == 38,] #event at 4.7 years
...
predLong.0<-predict(jm_dev.PP,newdata = predict.data.time.0,times = seq(0,3, length.out=20),return_newdata = T) #for future time point

predSurv.0<-predict(jm_dev.PP,newdata = predict.data.time.0,process = "event",times = seq(0,3, length.out=20),return_newdata = T) # use the information before 3 years to predict
...
predict.data.time.3<-predict.data[predict.data$followupyears<3,] 

predict.data.time.3$cens<-0 #not event

predict.data.time.3$years<-2.8 

#predict for longitudinal outcomes

predLong.3<-predict(jm_dev.PP,newdata = predict.data.time.3,times = seq(2.8,6, length.out=20),return_newdata = T) #for future time point

predSurv.3<-predict(jm_dev.PP,newdata = predict.data.time.3,process = "event",times = seq(2.8,6, length.out=20),return_newdata = T) # use the information before 3 years to predict
...
predict.data.time.4<-predict.data[predict.data$followupyears<4.2,] 

predict.data.time.4$cens<-0 #not event

predict.data.time.4$years<-4.2 

#predict for longitudinal outcomes

predLong.4<-predict(jm_dev.PP,newdata = predict.data.time.4,times = seq(4.2,7.2, length.out=20),return_newdata = T) #for future time point

predSurv.4<-predict(jm_dev.PP,newdata = predict.data.time.4,process = "event",times = seq(4.2,7.2, length.out=20),return_newdata = T) # use the information before 3 years to predict
...
png("JMpred0.CamPalGN.png",width = 3500,height =2000,res = 400)
...
png("JMpred3.CamPalGN.png",width = 3500,height =2000,res = 400)
...
png("JMpred4.CamPalGN.png",width = 3500,height =2500,res = 400)
...
```{r predict individual in PICNICS}
...
#random select idpicc==645 is kind of ok

predict.data<-IECV_vlong_PIC[IECV_vlong_PIC$idpicc == 645,] #event at 3.3 years
...
#-----From baseline up to 1.62 year-----

predict.data.time.1<-predict.data[predict.data$followupyears<2,] 

predict.data.time.1$cens<-0 #not event

predict.data.time.1$years<-1.62

predLong.1<-predict(jm_dev.PP,newdata = predict.data.time.1,times = seq(1.62,4.62, length.out=20),return_newdata = T) #for future time point

predSurv.1<-predict(jm_dev.PP,newdata = predict.data.time.1,process = "event",times = seq(1.62,4.62, length.out=20),return_newdata = T) # use the information before 3 years to predict
...
png("JMpred0.PICNICS.png",width = 3500,height =2000,res = 400)
...
png("JMpred1.PICNICS.png",width = 3500,height =2000,res = 400)
...
```{r predict individual in ParkWest}
...
predict.data<-IECV_vlong_PA[IECV_vlong_PA$idpicc == 499,] #event at 4.3 years
...
predLong.1<-predict(jm_dev.PP,newdata = predict.data.time.1,times = seq(1.03,4.03, length.out=20),return_newdata = T) #for future time point

predSurv.1<-predict(jm_dev.PP,newdata = predict.data.time.1,process = "event",times = seq(1.03,4.03, length.out=20),return_newdata = T) # use the information before 3 years to predict
...
predict.data.time.3<-predict.data[predict.data$followupyears<3.5,] 
...
predLong.3<-predict(jm_dev.PP,newdata = predict.data.time.3,times = seq(3.04,6.4, length.out=20),return_newdata = T) #for future time point

predSurv.3<-predict(jm_dev.PP,newdata = predict.data.time.3,process = "event",times = seq(3.04,6.04, length.out=20),return_newdata = T) # use the information before 3 years to predict

plot(predLong.3,predSurv.3,outcomes=1:2)

#----Arrange plot----

png("JMpred0.ParkWest.png",width = 3500,height =2000,res = 400)
...
png("JMpred1.ParkWest.png",width = 3500,height =2000,res = 400)

plot(predLong.1,predSurv.1,outcomes=1:2,ylab_long = c(" "," "),ylab_event = "Cumulative risk of dementia",xlab = "Follow-up years",cex_ylab_long = 1.5,cex_axis = 1.2,cex_xlab = 1)

dev.off()

png("JMpred2.ParkWest.png",width = 3500,height =2000,res = 400)
...
```{r NOT RUN idpicc is 323 NYPUM}


predict.data<-IECV_vlong_NY[IECV_vlong_NY$idpicc == 323,] #event at 7.5 years
...
plot(predMMSE.0,predSurv.0,outcomes=1:2)

#-----From baseline up to 1 year-----

predict.data.time.1<-predict.data[predict.data$followupyears<1,] 

predict.data.time.1$cens<-0 #not event

predict.data.time.1$years<-1

predMMSE.1<-predict(jm_dev.NP,newdata = predict.data.time.1,times = seq(1,4, length.out=20),return_newdata = T) #for future time point

predSurv.1<-predict(jm_dev.NP,newdata = predict.data.time.1,process = "event",times = seq(1,4, length.out=20),return_newdata = T) # use the information before 3 years to predict


plot(predMMSE.1,predSurv.1,outcomes=1:2)
...
plot(predMMSE.2,predSurv.2,outcomes=1:2)
...
predMMSE.3<-predict(jm_dev.NP,newdata = predict.data.time.3,times = seq(3,6, length.out=20),return_newdata = T) #for future time point

predSurv.3<-predict(jm_dev.NP,newdata = predict.data.time.3,process = "event",times = seq(3,6, length.out=20),return_newdata = T) # use the information before 3 years to predict

plot(predMMSE.3,predSurv.3,outcomes=1:2)
...
predMMSE.4<-predict(jm_dev.NP,newdata = predict.data.time.4,times = seq(4,7, length.out=20),return_newdata = T) #for future time point

predSurv.4<-predict(jm_dev.NP,newdata = predict.data.time.4,process = "event",times = seq(4,7, length.out=20),return_newdata = T) # use the information before 3 years to predict

plot(predMMSE.4,predSurv.4,outcomes=1:2)

#------From baseline up to 5 years-----

predict.data.time.5<-predict.data[predict.data$followupyears<5,] 
...
predMMSE.5<-predict(jm_dev.NP,newdata = predict.data.time.5,times = seq(5,8, length.out=20),return_newdata = T) #for future time point

predSurv.5<-predict(jm_dev.NP,newdata = predict.data.time.5,process = "event",times = seq(5,8, length.out=20),return_newdata = T) # use the information before 3 years to predict

plot(predMMSE.5,predSurv.5,outcomes=1:2)
...
predMMSE.6<-predict(jm_dev.NP,newdata = predict.data.time.7,times = seq(6,10, length.out=20),return_newdata = T) #for future time point

predSurv.6<-predict(jm_dev.NP,newdata = predict.data.time.7,process = "event",times = seq(6,10, length.out=20),return_newdata = T) # use the information before 3 years to predict

plot(predMMSE.6,predSurv.6,outcomes=1:2)

#------From baseline up to 7 years-----

predict.data.time.7<-predict.data[predict.data$followupyears<7,] 

predict.data.time.7$cens<-0 #not event

predict.data.time.7$years<-7

#predict for longitudinal outcomes

predMMSE.7<-predict(jm_dev.NP,newdata = predict.data.time.7,times = seq(7,10, length.out=20),return_newdata = T) #for future time point

predSurv.7<-predict(jm_dev.NP,newdata = predict.data.time.7,process = "event",times = seq(7,10, length.out=20),return_newdata = T) # use the information before 3 years to predict

plot(predMMSE.7,predSurv.7,outcomes=1:2)
