...
#use the baseline dataset to create the bootstrap dataset
...
for (i in 1:100) {
  ...
  for (j in 1:length(n_new)){
    n_len <- length(IECV_vlong_PA$idpicc[IECV_vlong_PA$idpicc %in% n_new[j]])
    newdata <- IECV_vlong_PA[IECV_vlong_PA$idpicc %in% n_new[j],]
  ...
  fit1.mmse <- lme(mmse~agebl+sex+yearseducation+cognitivebl+rcs(followupyears,3), random = ~ followupyears | idpicc,
                        data = trainingData)
  fit2.mds <- lme(MDSUPDRS3~agebl+sex+hybl+rcs(followupyears,3), random = ~ followupyears | idpicc, data=trainingData)
  
  Mixed_fit12.PA<-list(fit1.mmse,fit2.mds)
  
  survFit1 <- coxph(Surv(years,cens)~agebl+sex+hybl+cognitivebl+hallucinationsbl, 
                       data = trainingData.id, x = TRUE,model = TRUE)
  
  jointFit1 <- jm(survFit1,Mixed_objects=Mixed_fit12.PA, time_var = "followupyears")
