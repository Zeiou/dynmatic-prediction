...
IECV_vlong_NY%>%
  filter(cens==1)%>%
  filter(followupyears>3)

#random select idpicc==351

predict.data<-IECV_vlong_NY[IECV_vlong_NY$idpicc == 351,] #event at 8.5 years
...
predMMSE.1<-predict(jm_dev.NP,newdata = predict.data.time.1,times = seq(1.02,4, length.out=20),return_newdata = T) #for future time point

predSurv.1<-predict(jm_dev.NP,newdata = predict.data.time.1,process = "event",times = seq(1.02,4, length.out=20),return_newdata = T) # use the information before 3 years to predict
...
predict.data.time.2$years<-2.07

predMMSE.2<-predict(jm_dev.NP,newdata = predict.data.time.2,times = seq(2.07,5, length.out=20),return_newdata = T) #for future time point

predSurv.2<-predict(jm_dev.NP,newdata = predict.data.time.2,process = "event",times = seq(2.07,5, length.out=20),return_newdata = T) # use the information before 3 years to predict
