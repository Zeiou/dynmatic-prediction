...
data1[is.na(data1$years),] #One patients in PICNICS, idpicc, PCP66, only had baseline visit time and time of death, no followup information. 
...
21+8+4+29+14+62 #Without ICICE, so CamPalGN n=21; NYPUM n=8; ParkWest n=4; PICNICS=29+1; PINE n=76

1080-155 #Without ICICLE, so 927
```
...
#png("missingp.png",width = 2000,height =2000,res = 400)
...
nrow(data1.10[complete.cases(data1.10[,c(8,9,10,51,52)]),])  #816 in data1.10.remove 841 in data1.10.remove without smoking

(896-816)/896 #0.09 

(890-841)/890 #0.06

colnames(data1.10[,c(9,10,51,52)])
...
data.baseline.1$sex<-factor(data.baseline.1$sex,
                    levels = c(1,2),
                    labels= c("male","female"))

table(data.baseline.1$sex)
...
table(data1.10.remove$hallucinationsindex)
...
table(data1.10.remove$study,data1.10.remove$smoking)
...
data%>%
  ...
  filter(idpicc!=1271)%>%
  group_by(study)%>%
  count() #need to remove study 2 cos ICICLE is not inside
...
PICNICS<-data1.10.remove%>%
  filter(study=="PICNICS")

PICNICS.cens<-PICNICS%>%
  filter(cens==0)

PICNICS.event<-PICNICS%>%
  filter(cens==1)
